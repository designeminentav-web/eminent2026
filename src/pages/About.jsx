...
const About = () => {
  ...
  useEffect(() => {
    ...
    // Fallback timer (agar internet bahut slow ho)
  ...
  return (
    <>
      {/* SEO META TAGS */}
      <Helmet>
        <title>About Us | Eminent Audio Visual - Professional AV Solutions</title>
        <meta 
          name="description" 
          content="Learn more about Eminent Audio Visual. We deliver future-ready professional audio-visual systems, turnkey installations, and end-to-end technical support." 
        />
      </Helmet>
      ...
      <div className="bg-[#050B18] text-white overflow-hidden">
        ...
        <section className="relative min-h-screen w-full overflow-hidden flex items-center justify-center py-20 lg:py-32">
          ...
          <img
            ...
            loading="eager"
            fetchpriority="high" // <-- SPEED OPTIMIZATION (Loads first)
  ...
  By blending technical mastery with creative vision, we transform environments into immersive experiences, setting new benchmarks in audiovisual excellence.
        ...
        {/* MAIN CONTENT SECTION - WITH CONTENT VISIBILITY */}
        <div 
          className="max-w-7xl mx-auto px-5 sm:px-8 md:px-10 py-16 md:py-24"
          style={{ contentVisibility: "auto", containIntrinsicSize: "auto 1000px" }} // <-- SPEED OPTIMIZATION (Neeche ka hissa scroll hone par render hoga)
        >
          ...
          <section className="bg-gradient-to-br from-[#071028] via-[#0B1736] to-[#12386b] rounded-[30px] p-6 sm:p-10 md:p-12 mb-24 border border-white/5 shadow-2xl">
            ...
            >
              {[
                ...
                { icon: <FaAward />, value: 25, label: "Brands Partnered" },
                { icon: <FaUsers />, value: 1500, label: "Happy Clients" },
                { icon: <FaCheckCircle />, value: 10, label: "Years Experience" },
              ].map((item, i) => (
                <motion.div
                  key={i}
                  variants={itemVariants}
                  ...
                  >
                    <div className="w-full overflow-hidden h-[260px] sm:h-[300px]">
                      <motion.img
                        ...
                        loading="lazy"
                        decoding="async" // <-- SPEED OPTIMIZATION (Background processing)
